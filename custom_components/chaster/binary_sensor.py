...
def _contains_obedience(value) -> bool:
    """Return True when an extension entry identifies itself as Obedience."""
    if isinstance(value, dict):
        for key in (
            ...
            if isinstance(candidate, str) and candidate.strip().lower() == "obedience":
                return True
        return any(_contains_obedience(item) for item in value.values())
    if isinstance(value, list):
        return any(_contains_obedience(item) for item in value)
    return False
...
class ChasterObedienceBinary(_CurrentLockBinary):
    def __init__(self, coordinator, role):
        super().__init__(
            coordinator,
            role,
    ...
    def is_on(self):
        ...
        return _contains_obedience(lock.get("extensions", [])) if lock else False
    ...
    def extra_state_attributes(self):
        ...
        return {
            ...
            "extensions": lock.get("extensions", []) if lock else [],
        }
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for role in ("wearer", "keyholder"):
        entities.extend([
            ...
            ChasterObedienceBinary(coordinator, role),
        ])
