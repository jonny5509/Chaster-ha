...
class ChasterCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        try:
            ...
            for role, role_items in role_locks.items():
                active_items = [
                ...
                if active_lock and self.lock_id(active_lock):
                    try:
                        history_by_role[role] = self._dict_list(
                            await self.api.history(self.lock_id(active_lock))
                        )
                    except ChasterApiError as err:
                        _LOGGER.debug(
                            "Unable to retrieve %s lock history: %s", role, err
                        )
                        old_history = (
                            old_data.get("history_by_role", {}).get(role, [])
                            if isinstance(old_data.get("history_by_role"), dict)
                            else []
                        )
                        history_by_role[role] = self._dict_list(old_history)

            history = history_by_role[ROLE_WEARER] or history_by_role[ROLE_KEYHOLDER]
